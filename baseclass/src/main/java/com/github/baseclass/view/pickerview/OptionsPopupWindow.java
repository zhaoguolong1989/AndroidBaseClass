package com.github.baseclass.view.pickerview;
...
public class OptionsPopupWindow extends PopupWindow implements OnClickListener {
    ...
    public OptionsPopupWindow(Context context, String title) {// 没有title 传空
        super(context);
        this.setWidth(LayoutParams.FILL_PARENT);
        this.setHeight(LayoutParams.WRAP_CONTENT);
        this.setBackgroundDrawable(new BitmapDrawable());
        this.setOutsideTouchable(true);// 这样设置才能点击屏幕外dismiss窗口
        this.setAnimationStyle(R.style.timepopwindow_anim_style);

        LayoutInflater mLayoutInflater = LayoutInflater.from(context);
        rootView = mLayoutInflater.inflate(R.layout.pw_options, null);
        TextView pw_title = (TextView) rootView.findViewById(R.id.pw_title);
        if (!"".equals(title) && title.length() > 0) {
            pw_title.setText(title);
        } else {
            pw_title.setVisibility(View.GONE);
        }
